    ...
    <Box sx={{ width: "100%", maxWidth: 1200, mx: "auto", py: 3 }}>
      <Box sx={{ display: "flex", gap: 2, mb: 3 }}>
        ...
        >
          ...
          <Button
            ...
            onClick={() =>
              router.push(
                `/news/general/top?image=${encodeURIComponent(
                  topNews.image
                )}&title=${encodeURIComponent(
                  topNews.title
                )}&description=${encodeURIComponent(topNews.description)}`
              )
            }
          >
            Читати більше
      ...
      >
        {smallNews.map((news, i) => (
          <Box
            ...
            onClick={() =>
              router.push(
                `/news/general/small-${i}?image=${encodeURIComponent(
                  news.image
      ...
      {listNews.map((news, i) => (
        <Box
          ...
          onClick={() =>
            router.push(
              `/news/general/${i}?image=${encodeURIComponent(
                news.image
              )}&title=${encodeURIComponent(
                news.title
              )}&description=${encodeURIComponent(news.description)}&meta=${encodeURIComponent(
                news.meta
              )}`
            )
          }
